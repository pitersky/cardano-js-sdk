...
describe('SingleAddressWallet load', () => {
  ...
  it('tip value ignored while connection is down', async () => {
    ...
    const chainHistoryProvider = mocks.mockChainHistoryProvider2(MAX_FETCH_DURATION);
    const utxoProvider = mocks.mockUtxoProvider();
    const connectionStatusTracker$ = new ReplaySubject<ConnectionStatus>();
    ...
    // tip$ fetch triggered by ConnectionStatus & wallet settled
