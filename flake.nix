{
  inputs = {
    ...
    dream2nix.url = "github:blaggacao/dream2nix";
  };
      ...
      in
        lib.recursiveUpdate (
          ...
          in {
            ...
            add-type-notation.prePatch = lib.optionalString (n == "@cardano-sdk/core")
            ''
            ...
            fix-relative-references.postPatch = let
              ...
              '');
            in ''
              cp ${./tsconfig.json} ./src/tsconfig-root.json
              mkdir build
              cp ${./build/cjs-package.json} ./build/cjs-package.json
              cp ${./build/esm-package.json} ./build/esm-package.json
